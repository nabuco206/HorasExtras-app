...
new class extends Component {
    ...
    {
         \Log::info('Entró a RprobarSeleccionados 3'); 
        session()->flash('mensaje', '¡Método RprobarSeleccionados ejecutado!');

        $service = app(\App\Services\SolicitudHeService::class);
        $service->cambiarEstadoMultiple($this->seleccionados, 3, 'Aprobado masivamente');
    ...
    {
        \Log::info('Entró a AprobarSeleccionados 3'); 
        \Log::info('IDs seleccionados:', $this->seleccionados);
        session()->flash('mensaje', '¡Método AprobarSeleccionados ejecutado!');

        $service = app(\App\Services\SolicitudHeService::class);
        $service->cambiarEstadoMultiple($this->seleccionados, 4, 'Aprobado masivamente');
...
<section class="w-full" x-data="{
    ...
    toggleAll() {
        if (this.selectAll) {
            this.seleccionados = @json($solicitudes->pluck('id'));
        } else {
            this.seleccionados = [];
        }
    }
    ...
    <!-- <x-sistema.layout :heading="__('Ingreso Hora Extraaaaaaaa')"> -->
        <!-- Formulario -->
                ...
                <div class="p-6">
                    ...
                    <div class="mb-4 flex gap-2">
                        ...
                        <button
                            ...
                            id="btn-rechazar"
                            wire:click="rechazarSeleccionados"
                        >
                            Rechazo
                        </button>
                        <!-- <button x-on:click="console.log(seleccionados)">Ver seleccionados</button> -->
                    </div>
                                ...
                                <tr>
                                    <th class="px-2 py-2 text-center whitespace-nowrap">
                                        ...
                                        <input type="checkbox"
                                            x-model="selectAll"
                                            @change="toggleAll"
                                            :checked="seleccionados.length === {{ $solicitudes->count() }} && {{ $solicitudes->count() }} > 0"
